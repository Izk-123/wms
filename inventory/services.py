...
def issue_stock(item, warehouse, quantity, reference="", notes="", user=None):
    ...
    if stock.quantity < quantity:
        raise ValidationError(
            ...
            f"{item.unit.symbol}, Requested: {quantity} {item.unit.symbol}."
        )
    ...
    # ── Low stock check after every issue ───────────────
    if stock.is_low_stock:
        try:
            ...
            notify_users(
                ...
                message=(
                    f"{item.name} is now at {stock.quantity} "
                    f"{item.unit.symbol} in {warehouse.name}. "
                    f"Minimum is {item.minimum_stock} {item.unit.symbol}."
                ),
                notification_type="low_stock",
                level="danger",
                url=f"/inventory/items/{item.pk}/",
            )
        except Exception:
            pass
    # ────────────────────────────────────────────────────
