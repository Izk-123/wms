# documents/services/quotation.py
from .base import BasePDFService
from reportlab.lib import colors
from reportlab.lib.units import cm
from reportlab.platypus import Paragraph, Spacer, Table, TableStyle

class QuotationPDFService(BasePDFService):
    ...
    def build_body(self, story):
        ...
        story.append(Paragraph("QUOTATION", self.styles['CompanyHeading']))
        ...
        story.append(Paragraph(f"Valid Until: {obj.valid_until.strftime('%d %B %Y') if obj.valid_until else 'N/A'}", self.styles['Normal']))
        story.append(Spacer(1, 0.5*cm))

        # Customer
        story.append(Paragraph("Customer:", self.styles['CompanyHeading']))
        ...
        grand_total = subtotal - (obj.discount_amount or 0)
